...
class SitNewEnv(HumanoidEnv):
    def __init__(self, cfg) -> None:
        ...
        # 保持与sit_simple相同的总观测维度，但改进goal_obs的内容
        self.num_prop_obs = 15 * (3 + 6 + 3 + 3) - 2  # 保持与sit_simple一致：223
        self.num_obj_obs = 15  # 保持物体观测：15  
        self.num_goal_obs = 9  # 目标观测维度保持9，但内容将包含椅子朝向信息
        self.num_obs = self.num_prop_obs + self.num_obj_obs + self.num_goal_obs  # 总计：247
    ...
    def obs_space(self): #todo: 设置obs_space
        return {
            'obs' : self.num_prop_obs + self.num_obj_obs + self.num_goal_obs,
            'bps' : (self.num_bps, 3)
        }
    ...
    def compute_guidance(self, env_ids=None):
        ...
        # 在椅子正面0.5米处设置引导点，机器人应该到这里然后背对椅子坐下
    ...
    def compute_goal_obs(self, root_pos, root_rot, goal_pos, goal_rot, obj_rot):
        ...
        # 简化目标朝向表示：只用Z轴旋转角度(1维)代替完整的6维切线-法线表示
        local_goal_rot = torch_utils.quat_mul(heading_rot, goal_rot)
        # 提取Z轴旋转角度
        goal_yaw = torch.atan2(2.0 * (local_goal_rot[:, 0] * local_goal_rot[:, 1] + local_goal_rot[:, 2] * local_goal_rot[:, 3]),
                              1.0 - 2.0 * (local_goal_rot[:, 1] * local_goal_rot[:, 1] + local_goal_rot[:, 2] * local_goal_rot[:, 2]))
        goal_yaw = goal_yaw.unsqueeze(-1)
        ...
        # 添加距离目标的距离信息 (1维)
        goal_distance = torch.norm(local_goal_pos, dim=-1, keepdim=True)
        
        # 总计: local_goal_pos(3) + goal_yaw(1) + chair_facing_2d(2) + goal_distance(1) + 2维填充 = 9维
        padding = torch.zeros((local_goal_pos.shape[0], 2), device=local_goal_pos.device, dtype=local_goal_pos.dtype)
        
        obs = torch.cat((
            local_goal_pos,      # 3维：目标位置
            goal_yaw,           # 1维：目标朝向
            chair_facing_2d,    # 2维：椅子朝向
            goal_distance,      # 1维：到目标距离  
            padding,            # 2维：填充到9维
            ),    
        dim = -1)
        return obs
