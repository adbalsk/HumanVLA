...
class SitNewEnv(HumanoidEnv):
    ...
    def compute_guidance(self, env_ids=None):
        ...
        chair_forward = torch_utils.quat_rotate(obj_rot, torch.tensor([0.0, 1.0, 0.0], device=obj_rot.device, dtype=obj_rot.dtype).expand_as(obj_pos))
    ...
    def compute_goal_obs(self, root_pos, root_rot, goal_pos, goal_rot, obj_rot):
        ...
        chair_forward_global = torch_utils.quat_rotate(obj_rot, torch.tensor([0.0, 1.0, 0.0], device=obj_rot.device, dtype=obj_rot.dtype).expand_as(root_pos))
    ...
    def compute_location_reward(self, root_pos, prev_root_pos, root_rot, prev_root_rot, object_root_pos, tar_pos, tar_speed, dt,
        ...
        # 计算椅子正面位置
        chair_forward = torch_utils.quat_rotate(obj_rot, torch.tensor([0.0, 1.0, 0.0], device=obj_rot.device, dtype=obj_rot.dtype).expand_as(object_root_pos))
        approach_distance = 0.5
        ...
        robot_forward = torch_utils.quat_rotate(root_rot, torch.tensor([1.0, 0.0, 0.0], device=root_rot.device, dtype=root_rot.dtype).expand_as(root_pos))
