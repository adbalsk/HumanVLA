...
def flexible_load_state_dict(model, ckpt_state_dict, verbose=True):
    """
    尝试把 ckpt_state_dict 的权重填入 model。
    - 对于 name 相同且 shape 相同的参数，直接复制；
    - 对于 name 相同但 shape 不同且 ndim 相同，按重叠部分 slice copy；
    - 忽略 ckpt 多余的 key；保留 model 中 ckpt 没有的 key（随机初始化）。
    返回 new_state_dict, report
    """
    ...
    report = {
        ...
        'missing_in_ckpt': [],
        'unexpected_in_ckpt': [],
        'shape_mismatch_kept_model': []
    }

    # collect unexpected
    for k in ckpt_state_dict.keys():
        if k not in model_state:
            report['unexpected_in_ckpt'].append(k)

    for k, m_val in model_state.items():
        if k in ckpt_state_dict:
            ck_val = ckpt_state_dict[k]
            # identical shape -> copy
            if hasattr(ck_val, 'shape') and tuple(ck_val.shape) == tuple(m_val.shape):
                new_state[k] = ck_val
                report['copied_exact'].append(k)
            else:
                # if same number of dimensions, try partial copy on overlapping slices
                if hasattr(ck_val, 'ndim') and hasattr(m_val, 'ndim') and ck_val.ndim == m_val.ndim:
                    tmp = m_val.clone()
                    slices = tuple(slice(0, min(int(ck_val.shape[i]), int(m_val.shape[i]))) for i in range(ck_val.ndim))
                    try:
                        tmp[slices] = ck_val[slices]
                        new_state[k] = tmp
                        report['partial_copied'].append((k, tuple(ck_val.shape), tuple(m_val.shape)))
                        if verbose:
                            _logger.warning("Partial-loaded param %s from shape %s into model shape %s", k, tuple(ck_val.shape), tuple(m_val.shape))
                    except Exception as e:
                        new_state[k] = m_val
                        report['shape_mismatch_kept_model'].append((k, getattr(ck_val, 'shape', None), getattr(m_val, 'shape', None), str(e)))
                        if verbose:
                            _logger.warning("Failed partial copy for %s: %s", k, str(e))
                else:
                    new_state[k] = m_val
                    report['shape_mismatch_kept_model'].append((k, getattr(ck_val, 'shape', None), getattr(m_val, 'shape', None)))
                    if verbose:
                        _logger.warning("Shape or ndim mismatch for %s: ckpt %s vs model %s. Skipping ckpt param.", k, getattr(ck_val, 'shape', None), getattr(m_val, 'shape', None))
        else:
            new_state[k] = m_val
            report['missing_in_ckpt'].append(k)

    return new_state, report

class AMPStage2Trainer:
    ...
    def compute_loss(self, batch_dict):
        with torch.cuda.amp.autocast(enabled=self.auto_mixed_precision):
            ...
            #################################################################################################### actor loss
            ratio = torch.exp(batch_dict['neglogp'] - train_meta['neglogp']) # 等价于 π_new/π_old
            #surrogate: 裁剪
    ...
    def load_ckpt(self,path):
        ...
        ckpt_weights = ckpt.get('weight', ckpt)  # 尝试从 ckpt 中获取 'weight' 键的值；如果 ckpt 中没有 'weight' 键（比如 checkpoint 本身就是模型的 state_dict），则直接返回 ckpt 本身。
        new_state, report = flexible_load_state_dict(self.network, ckpt_weights, verbose=True)
        
        _logger.warning("Checkpoint load report: copied_exact=%d, partial=%d, missing=%d, unexpected=%d, kept_mismatch=%d",
                     len(report['copied_exact']), len(report['partial_copied']), len(report['missing_in_ckpt']),
                     len(report['unexpected_in_ckpt']), len(report['shape_mismatch_kept_model']))
        
        print("=== report summary ===")
        print({k: (len(v) if isinstance(v, list) else v) for k,v in report.items()})

        # print missing keys
        print("\n=== Missing keys (model expects these but ckpt doesn't have) ===")
        for k in report['missing_in_ckpt']:
            print(k)

        # print unexpected keys (ckpt had but model doesn't use)
        print("\n=== Unexpected keys in ckpt (ckpt has these but model doesn't) ===")
        for k in report['unexpected_in_ckpt'][:200]:
            print(k)

        # 最后加载到模型（strict=False 以防还有微小不匹配）
